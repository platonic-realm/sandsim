...
- Click a swatch in the on-screen palette to pick a material (or keys `0`-`9`:
  `0` Eraser · `1` Wall · `2` Sand · `3` Water · `4` Gas · `5` Oil · `6` Fire · `7` Lava · `8` Steam · `9` Wood · `P` Plant · `A` Acid · `M` Smoke · `G` Glass · `I` Ice · `S` Spring · `T` TNT · `H` Ash · `V` Volcano · `X` Void · `D` Mud · `Z` Virus · `E` Spark · `O` Obsidian · `L` Salt · `N` Snow · `Q` Mercury · `B` Gunpowder · `K` Thermite · `F` Frost)
