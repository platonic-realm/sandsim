/*
 * sandsim - connected SIMD streamed world, all materials (AVX2 single-grid)
 *
...
#include "simd_core.h"   // SimdOps + simdStep (SSE/AVX), Material enum
#include <cstdint>
#include <cstdio>
#include <cstring>
#include <cstdlib>
...
// `Material` and the SIMD ops live in simd_core.h.
using Ops = AvxOps;
...
public:
    ...
    void step() {
        simdStep<Ops>(grid.data(), moved.data(), SW, X0, X1, Y0, Y1, frame);
        ++frame;
    }
...
static int runBench(int steps, int wbox, int hbox) {
    ...
    printf("RESULT impl=cpp_world_avx rule=world_simd window=%dx%d wbox=%d hbox=%d steps=%d "
...
static int runInteractive() {
    ...
    SDL_Window* window = SDL_CreateWindow(
        "Connected SIMD World (AVX2) - arrows pan  [1]Wall [2]Sand [3]Water [4]Gas [0]Eraser",
