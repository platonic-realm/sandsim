...
.PHONY: all help c rust zig cpp opengl hip cuda vulkan bench materials bench-materials clean
...
zig:
	cd zig && $(ZIG_BUILD) && $(ZIG_BUILD_MAT)
cpp:
	$(MAKE) -C cpp
opengl:
	$(MAKE) -C opengl
...
all:
	...
	@if command -v zig   >/dev/null 2>&1; then cd zig && $(ZIG_BUILD) && $(ZIG_BUILD_MAT); else echo "skip zig    (no zig)";   fi
...
clean:
	-$(MAKE) -C c clean
	-cd rust && cargo clean
	-rm -f zig/sandsim zig/sandsim_materials
	-$(MAKE) -C cpp clean
	-$(MAKE) -C opengl clean
	-$(MAKE) -C hip clean
	-$(MAKE) -C cuda clean
